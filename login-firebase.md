...
### **Bước 2: Lấy Service Account và cấu hình biến môi trường Backend**

1. Vào **Project Settings** (biểu tượng bánh răng cạnh _Project Overview_).
2. Chuyển sang tab **Service accounts**.
3. Chọn **Node.js** và nhấn nút **Generate new private key** -> Xác nhận tải về file JSON.
4. Mở file JSON, copy **toàn bộ nội dung trên một dòng** và gán cho biến `FIREBASE_SERVICE_ACCOUNT`.
5. Khi chạy local, thêm biến vào `.env`. Khi deploy Railway, vào service -> **Variables** -> **New Variable**, đặt tên `FIREBASE_SERVICE_ACCOUNT` và dán toàn bộ JSON làm giá trị.
6. Không commit, upload hoặc tạo file service-account trong project. Sau khi đã lưu biến môi trường, hãy xóa file JSON tải về khỏi máy nếu không còn cần dùng.
...
_Đừng quên export `googleLogin` trong `src/controllers/auth.controller.js`._
...
1. Cài đặt thư viện Firebase Client SDK:
...
- [ ] `FIREBASE_SERVICE_ACCOUNT` chứa đầy đủ JSON service account, được đặt trong `.env` local hoặc Railway Variables và không bị commit.
